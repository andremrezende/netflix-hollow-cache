...
- **HollowConsumerService**: Manages the synchronization of the consumer cache with the Hollow repository. Uses the **Type-Safe Generated API** for efficient and safe data access.
- **AgenciaController**: Provides endpoints for accessing cached data.
- **GenerateHollowAPI**: Utility class to generate the type-safe API from the data model.

## Hollow API Generator

This project uses **Hollow's Code Generation** to create a type-safe API for consuming data. This approach provides:

- ✅ **Compile-time safety**: Errors caught during compilation, not at runtime
- ✅ **Better performance**: No reflection overhead
- ✅ **IDE autocomplete**: Full IntelliSense support
- ✅ **Cleaner code**: Direct access to fields via methods like `getId()`, `getNome()`

### How to Regenerate the API

If you modify the `Agencia` model class, you need to regenerate the API.

#### Option 1: Using the automated script (Recommended)

```bash
# Regenerate API automatically
./regenerate-api.sh
```

This script handles the entire process:
1. Removes old API classes
2. Temporarily moves dependent files
3. Generates new API
4. Restores files
...
# 2. Move dependent files temporarily
...
The regeneration creates/updates classes in `src/main/java/br/com/rezende/cache/hollowcacheagencia/consumer/api/`:
- `AgenciaAPI.java` - Main API class
- `Agencia.java` - Type-safe wrapper for Agencia objects
- `AgenciaPrimaryKeyIndex.java` - Primary key index for fast lookups
- Other supporting classes (18+ classes total)

**Important**: Commit the generated `consumer/api/` folder to git. This is the [recommended practice by Netflix](https://netflix.github.io/hollow/getting-started/#consumer-api-generation).

### Consumer Implementation

The `HollowConsumerService` uses the generated API instead of generic Hollow objects:

```java
// OLD: Using GenericHollowObject (reflection-based)
GenericHollowObject obj = new GenericHollowObject(stateEngine, "Agencia", ordinal);
int id = obj.getInt("id");
String nome = obj.getString("nome");

// NEW: Using Generated API (type-safe)
...
## How to Test
1. **Start the application**: `mvn spring-boot:run` or `docker-compose up --build`
2. **Publish initial data**: `curl -X POST http://localhost:8080/publicar`
3. **Access consumer endpoints**: `curl http://localhost:8080/agencias`
4. **Run unit tests**: `mvn test`

## Available Endpoints

### Producer Endpoints
- `POST /publicar` - Publishes 100 agencies to Hollow
- `POST /carga-pesada` - Publishes 100,000 agencies (performance test)
- `POST /alterar-cinco-agencias` - Updates the first 5 agencies
- `POST /remover-99-agencias` - Removes 99 agencies (or all but one if less than 100)
- `POST /remover-agencias` - Removes all agencies

### Consumer Endpoints
- `GET /agencias` - Lists all agencies
- `GET /agencias/{id}` - Fetches agency by ID
- `GET /agencias/paginado?page={page}&size={size}` - Paginated list
- `GET /metricas/ocupacao-memoria` - Memory usage metrics

## Hollow Explorer UI

The project includes Hollow Explorer UI for inspecting and visualizing Hollow data artifacts. When you start the application, the Explorer UI automatically starts on port **7777**.

**Access:** [http://localhost:7777](http://localhost:7777)

### Features:
- Browse data models and schemas
- Inspect individual records and their fields
- View type hierarchies and relationships
- Analyze data distribution and statistics
- Navigate through different versions/snapshots
- Query data using the built-in explorer

### Usage:
1. Start the application: `mvn spring-boot:run`
2. Publish some data: `curl -X POST http://localhost:8080/publicar`
3. Open the Explorer UI: [http://localhost:7777](http://localhost:7777)
4. Select the "agencia" data model to explore

The Explorer UI is particularly useful for:
- Debugging data issues
- Understanding the data structure
- Validating data after migrations or updates
- Learning how Hollow stores and organizes data
